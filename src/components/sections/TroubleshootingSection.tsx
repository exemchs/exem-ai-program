import { AlertCircle, Play } from "lucide-react";
import { OS_KEYS, type OsType } from "../../constants/os";

export default function TroubleshootingSection({ os }: { os: OsType }) {
  const keys = OS_KEYS[os];
  return (
    <section className="py-24 bg-[#EFF6FF] border-y border-[#E2E5EB]">
      <div className="max-w-5xl mx-auto px-6">
        <div className="flex items-center gap-4 mb-8">
          <AlertCircle className="text-[#3B82F6]" size={32} />
          <h2 className="text-3xl font-normal text-[#1a2234]">
            잠시 멈추고 싶다면
          </h2>
        </div>

        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-white border border-[#E2E5EB]">
            <h3 className="text-lg font-normal text-[#1a2234] mb-4 flex items-center gap-2">
              <Play size={18} className="text-green-500" /> 다시 시작할 때
            </h3>
            <ol className="list-decimal list-inside text-[#6B7280] space-y-2">
              <li>Cursor 실행</li>
              <li>
                <code className="text-[#3B82F6] bg-[#EFF6FF] px-1.5 py-0.5 rounded">
                  {keys.openTerminal}
                </code>{" "}
                터미널 열기
              </li>
              <li>
                <code className="text-[#3B82F6] bg-[#EFF6FF] px-1.5 py-0.5 rounded">
                  claude
                </code>{" "}
                입력
              </li>
              <li>"이어서 해줘" 또는 /(원하는 day) 입력</li>
            </ol>
          </div>

          <div className="p-6 rounded-2xl bg-white border border-[#E2E5EB]">
            <h3 className="text-lg font-normal text-[#1a2234] mb-4 flex items-center gap-2">
              <AlertCircle size={18} className="text-red-500" /> 이어하기가 안
              될 때
            </h3>
            <p className="text-[#6B7280] mb-4">
              증상:{" "}
              <code className="text-red-500 bg-red-50 px-1.5 py-0.5 rounded">
                command not found: claude
              </code>
            </p>
            <p className="text-sm text-[#9CA3AF] mb-4">
              Cursor에서 새 터미널을 열면 발생할 수 있어요.
            </p>
            <ul className="list-disc list-inside text-[#6B7280] space-y-2">
              <li>
                방법 1: {keys.osLabel}라면{" "}
                <code className="text-[#3B82F6] bg-[#EFF6FF] px-1.5 py-0.5 rounded">
                  {keys.shellReload}
                </code>{" "}
                또는 터미널 재시작
              </li>
              <li>방법 2: Claude Code 재설치</li>
              <li>
                방법 3: Cursor를 완전히 종료(
                <code className="text-[#3B82F6] bg-[#EFF6FF] px-1.5 py-0.5 rounded">
                  {keys.quit}
                </code>
                ) 후 다시 실행
              </li>
            </ul>
          </div>
        </div>
      </div>
    </section>
  );
}
